...
def get_args_parser():
    ...
    parser.add_argument('--v', default=20, type=int, help='Save checkpoint every x epochs.')
...
def train_dino(args):
    ...
    print(f"Student and Teacher are built: they are both {args.arch} network.", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,
                                      ...
                                      epoch, fp16_scaler, student_backbone, args)
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
                    ...
                    fp16_scaler, student_backbone, args):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):

        images[-1] = generate_attack(images[-1], student_backbone)
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        student_out = student_out.chunk(3) #self.ncrops
...
class DataAugmentationDINO(object):
    ...
    def __call__(self, image):
        ...
        crops.append(self.naive_transfo1(image))
        return crops
